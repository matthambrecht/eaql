...
fn integration_test_db_destroy_normal() {
    ...
    assert_eq!(engine("delete database db1!"), Ok("DROP DATABASE db1;".to_string()));
    
    // Multiple databases tests
    assert_eq!(engine("delete databases db1, db2, db3;"), Ok("DROP DATABASE db1, db2, db3;".to_string()));
    assert_eq!(engine("Destroy the databases db1, db2 and db3."), Ok("DROP DATABASE db1, db2, db3;".to_string()));
}
...
fn integration_test_table_accessor_normal_get() {
    ...
    assert_eq!(engine("get everything from test_table;"), Ok("SELECT * FROM test_table;".to_string()));
    
    // Test "get" keywords
    assert_eq!(engine("get all from test_table;"), Ok("SELECT * FROM test_table;".to_string()));
    assert_eq!(engine("retrieve all from test_table;"), Ok("SELECT * FROM test_table;".to_string()));
    assert_eq!(engine("find all from test_table;"), Ok("SELECT * FROM test_table;".to_string()));

    // Test column listing
    assert_eq!(engine("get me id and value from test_table."), Ok("SELECT id, value FROM test_table;".to_string()));
    assert_eq!(engine("get me id, price, value from test_table!"), Ok("SELECT id, price, value FROM test_table;".to_string()));
}
...
fn integration_test_table_accessor_normal_filter() {
    ...
    assert_eq!(engine("get all from test_table whenever id = 3;"), Ok("SELECT * FROM test_table WHERE id = 3;".to_string()));
...
fn integration_test_table_accessor_error_filter() {
    ...
    assert!(engine("get all from test_table where id = 3 or (price <= 2 and name is id)!").is_err());
...
fn integration_test_table_accessor_error_postprocessor() {
    ...
    assert!(engine("get all from test_table then limit 5 where id = 3 or (price <= 2 and name is \"3\").").is_err());
}
