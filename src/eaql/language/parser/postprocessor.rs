    ...
    pub fn parse_node(
        ...
        return Ok(LimitNode {
            limit: match tokens[*idx].literal.parse::<i32>() {
                Ok(state) => state,
                Err(_) => return Err(
                    format!("Limit post-processor expects 32-bit integer, got -> {}", tokens[*idx].lexeme)
                )
            },
    ...
    pub fn recurse_build(
        ...
        if next_token == TokenType::EoqToken {
            *idx += 1;
            ...
            PostProcessorNode::handle_postprocessor(
                tokens,
                final_node,
                depth,
                idx)?;
 
            PostProcessorNode::recurse_build(
                tokens,
                final_node,
                depth,
                idx
